...
def run_inference(cfg: experiment_manager.CfgNode, site: str, year: int):
    ...
    for i in tqdm(range(len(dataset))):
        ...
        label = patch['y'].cpu().numpy().astype('uint8')
    ...
    label_file = out_folder / f'label_{site}{dataset.year}_{cfg.NAME}.tif'
    geofiles.write_tif(out_file, prob_output, transform, crs)
    geofiles.write_tif(label_file, label_output, transform, crs)
