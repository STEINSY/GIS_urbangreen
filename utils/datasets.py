...
class AbstractDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, cfg):
        ...
        available_s2_bands = ['B2', 'B3', 'B4','B8']
    ...
    def _get_indices(bands, selection):
        # return [bands.index(band) for band in selection if band in bands]
        indices = []
        for band in selection:
            try:
                ...
                # 处理波段不存在于可用波段列表中的情况，例如跳过当前波段
                print(f"Warning: Band {band} not found in available bands. Skipping this band.")
        return indices
...
class TrainDataset(AbstractDataset):
    ...
    def __init__(self, cfg, run_type: str, no_augmentations: bool = False):
        ...
        for site in self.sites:
            site_samples_file = self.root_path / site / f'{site}2018_samples.json'
...
class TilesInferenceDataset(AbstractDataset):
    ...
    def __init__(self, cfg, site: str, year: int, include_label: bool = False):
        ...
        # getting all files
        samples_file = self.root_path / site / f'{site}{year}_samples.json'
